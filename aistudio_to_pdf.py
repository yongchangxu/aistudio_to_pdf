...
def extract_turns(data):
    """从 AI Studio 内部接口响应中提取对话轮次 (role + text)。"""
    turns = []
    ...
    while time.time() < deadline:
        ...
        if best_turns and len(best_turns) >= 2 and any(t["role"] == "user" for t in best_turns):
            ...
            try:
                ...
                cur_url = ""
            if "accounts.google.com" in cur_url and not login_hinted:
                log("需要登录或选择账号: 请在浏览器窗口中登录 Google, 或点选该对话所属的账号, 完成后自动继续。")
                login_hinted = True
            elif (time.time() - start > 15 and "aistudio.google.com" in cur_url
                  and not switch_hinted):
                log("页面已打开但未捕获到对话数据, 当前登录的账号可能不是这个对话的主人。")
                log("  → 请点击浏览器窗口右上角的账号头像, 切换到对话所属的账号;")
                log("    如果列表里没有, 选「添加其他账号」登录一次 (以后就都记住了)。")
                log("  切换完成后脚本会自动继续。")
                switch_hinted = True
...
def build_html(title, turns, model_name, url, include_thinking):
    ...
    with sync_playwright() as p:
        ...
        try:
            for idx, url in enumerate(args.urls, 1):
                ...
                title = extract_title(payload) or dom_title or "AI Studio 对话"
